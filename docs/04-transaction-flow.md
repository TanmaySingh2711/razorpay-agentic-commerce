...
The end-to-end flow as it runs today: the sequence, the owner of each step, and
where trust changes hands. Every step below is implemented. For the same flow
with the architecture diagram and the trust zones drawn out, see
...
sequenceDiagram
  ...
  MER-->>TX: server-read product (price, currency, stock) → PRODUCT_VERIFIED
...
| 5   | Product verification         | Product Decision Engine, re-reading through the Merchant Service's catalog boundary | Agent claim → source of truth          | `productId` in, the current product row out                        | Price, currency and stock re-read from PostgreSQL. **The agent's claimed price is discarded here.**                                                   |
